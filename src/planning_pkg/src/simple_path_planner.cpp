...
#include "nav_msgs/msg/odometry.hpp" // Now used for subscribing to F1TENTH pose
...
public:
    ...
    {
        // Subscriber: Subscribes to the robot's current pose from F1TENTH system
        ...
        // Publisher: Publishes the calculated path to control node
        path_publisher_ = this->create_publisher<nav_msgs::msg::Path>("/planning/path", 10);

        // Timer to periodically publish the path (2 seconds)
...
private:
    ...
    {
        // [TODO]: A* or RRT algorithm will calculate a dynamic path here.
    }
    ...
    void publish_path_fixed()
    {
        ...
        // Fixed 2 Waypoints: (6.0, 1.0) -> (5.0, 0.0)
