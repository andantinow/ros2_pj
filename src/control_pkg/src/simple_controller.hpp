...
private:
  ...
  // 레이스 라인 폭: 0.8m 이상 유지
  double collision_threshold_ = 0.8;      // Distance to trigger reverse (meters) - front collision
  double reverse_speed_ = 0.4;            // Gentle reverse speed (m/s)
  double reverse_duration_ = 1.5;         // How long to reverse (seconds)
  double side_collision_threshold_ = 0.7; // Side obstacle threshold (meters) - wider margin
  ...
  double side_avoidance_gain_ = 0.8;        // 측면 회피 강도 (increased for better side avoidance)
